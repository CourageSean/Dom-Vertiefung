...
const rowInput = document.getElementById("zeilenFeld");
const colomnInput = document.getElementById("spaltenFeld");
const resultDiv = document.getElementById("result");
resultDiv.appendChild(table2);
rowArr = [];

const generateTable = () => {
  for (let i = 1; i <= rowInput.value; i++) {
    const tr2 = document.createElement("tr");
    table2.appendChild(tr2);
    for (let j = 1; j <= colomnInput.value; j++) {
      const td2 = document.createElement("td");
      tr2.appendChild(td2).innerText = `${i}.${j}`;
    }
  }
};
...
const div1 = document.querySelector("section");

btn.addEventListener("click", () => {
  let randomUrl = data[Math.floor(Math.random() * data.length)].download_url;
...
const umwickler = document.querySelector(".umwickeln");
const innerUmwickler = document.createElement("div");
...
const speichern = document.getElementById("enter");
const userInput = document.getElementById("userinput");
const einkaufListe = document.getElementsByClassName("einkaufsliste");
speichern.addEventListener("click", () => {
  let li = document.createElement("li");
  li.innerText = userInput.value;
  einkaufListe[0].appendChild(li);
});
...
const unluckyFn = () => {
  ...
  for (let i = 0; i < 12; i++) {
    d.setFullYear(yearInput.value, i, 13);
    if (d.getDay() == 5) {
      console.log(i);
      let lii = document.createElement("li");
      yearUl.appendChild(lii);
      lii.textContent = `Friday, 13th ${month[i]} ${yearInput.value}`;
...
btn3.addEventListener("click", unluckyFn);
